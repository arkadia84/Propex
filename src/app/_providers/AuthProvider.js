...
const authProviderHandlers = {
  ...
  XELLAR: async ({ setConnected }) => {
    // Handle Xellar login logic here
    setConnected(true); // Assume connected for Xellar on success
    toast.success("Xellar provider selected");
  },

  DEMO: async ({ setConnected, setIsDemoMode, setCreateProfileModal }) => {
    try {
      setConnected(true); // Mark demo user as connected

      // connect to web3 using hardcoded private key from env DEMO_WALLET_PK
      const provider = new JsonRpcProvider(MAINNET_LISK_CONFIG.rpcTarget);
      const demoWallet = new Wallet(
        process.env.NEXT_PUBLIC_DEMO_WALLET_PK,
        provider
      );
      ...
      localStorage.setItem(
        "token",
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.._blD_rQe76xyslUtGrURPzEjJ4m0TcxPlStv9Fvx-5Y"
      );
      ...
      console.log(demoWallet.address);
